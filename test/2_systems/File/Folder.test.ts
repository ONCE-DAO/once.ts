...
import Folder from "../../../src/2_systems/File/Folder.class.mjs"
...
describe("When searching for files in folder", () => {
    test("folder should return all files", () => {
        let folder = new Folder(testDir.toString());
        expect(folder.exists).toBeTruthy()
        expect(folder.getFilesByExtentions().length).toBe(25)
    })
    test("folder should return all filtered files", () => {
        let folder = new Folder(testDir.toString());
        expect(folder.exists).toBeTruthy()
        expect(folder.getFilesByExtentions([".js"]).length).toBe(5)
    })
    test("folder should return all multi filtered files", () => {
        let folder = new Folder(testDir.toString());
        expect(folder.exists).toBeTruthy()
        expect(folder.getFilesByExtentions([".js", ".ts"]).length).toBe(15)
    })
})
...
describe("When searching recursive for files in folder", () => {
    test("folder should return all files", () => {
        let folder = new Folder(testDir.toString());
        expect(folder.exists).toBeTruthy()
        expect(folder.getFilesByExtentions(undefined,true).length).toBe(75)
    })
    test("folder should return all filtered files", () => {
        let folder = new Folder(testDir.toString());
        expect(folder.exists).toBeTruthy()
        expect(folder.getFilesByExtentions([".js"],true).length).toBe(15)
    })
    test("folder should return all multi filtered files", () => {
        let folder = new Folder(testDir.toString());
        expect(folder.exists).toBeTruthy()
        expect(folder.getFilesByExtentions([".js", ".ts"],true).length).toBe(45)
    })
})
