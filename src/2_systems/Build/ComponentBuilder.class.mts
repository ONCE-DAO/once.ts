...
export default class DefaultComponentBuilder implements ComponentBuilder {
    ...
    private constructor(path: string, submodule: SubmoduleInterface & Buildable, npmPackage: BuildableNpmPackage, typescriptProject: TypescriptProjectInterface) {
        this.path = path;
    ...
    install = async (config: BuildConfig) => this.buildables.forEach(async (buildable) => await buildable.install(config))
    beforeBuild = async (config: BuildConfig) => this.buildables.forEach(async (buildable) => await buildable.beforeBuild(config))
    build = async (config: BuildConfig) => this.buildables.forEach(async (buildable) => await buildable.build(config))
    afterBuild = async (config: BuildConfig) => this.buildables.forEach(async (buildable) => await buildable.afterBuild(config))
    ...
    static async init(submodule: SubmoduleInterface): Promise<ComponentBuilder> {
        ...
        return new DefaultComponentBuilder(submodule.path, submodule, npmPackage, typescriptProject)
    }
